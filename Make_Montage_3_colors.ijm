...
#@ File (label = "Directory DAPI", style = "directory") input_DAPI
...
if (file_list_488.length != file_list_647.length) {
    ...
    for (i = 0; i < file_list_488.length; i++) {
        ...
        // open the "DAPI" image
        run("Bio-Formats", "open=[" + input_DAPI + File.separator + file_list_DAPI[i] + "] autoscale color_mode=Default rois_import=[ROI manager] split_channels view=Hyperstack stack_order=XYCZT");     
        // Transform images to 8-bit
        run("8-bit");       
        // Subtract Backrgound
        run("Subtract Background...");        
        ...
        run("Merge Channels...", "c2=[" + target_green + "] c3=[" + target_blue + "] c6=[" + target_red + "] create keep");
